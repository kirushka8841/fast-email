...
def check_mail(mail=''):
    ...
    if length == 0:
        print('[INFO] На почте пока нет новых сообщений. Проверка происходит автоматически каждые 30 секунд!')
    else:
        id_list = []
        ...
        print(f'[+] У вас {length} входящих! Почта обновляется автоматически каждые 30 секунд!')
        ...
        for i in id_list:
            ...
            date = r.get('date')
            content_encoded = r.get('htmlBody')
            content = html.unescape(content_encoded)
